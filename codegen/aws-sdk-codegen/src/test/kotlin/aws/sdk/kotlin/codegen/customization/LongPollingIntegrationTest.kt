...
class LongPollingIntegrationTest {
    ...
    private fun generateClient(model: software.amazon.smithy.model.Model): Pair<aws.smithy.kotlin.codegen.test.TestContext, String> {
        ...
        val code = ctx.manifest.expectFileString("/src/main/kotlin/com/test/DefaultTestClient.kt")
        return ctx to code
    }
    ...
    private fun assertOperationHasLongPolling(model: software.amazon.smithy.model.Model, methodName: String, inputType: String, outputType: String) {
        val (_, code) = generateClient(model)
