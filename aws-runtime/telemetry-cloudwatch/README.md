...
Settings live on the object that owns the behaviour: aggregation on `SdkTelemetryProvider`,
...
class Handler : RequestHandler<Input, Output> {
    ...
    private val telemetry = SdkTelemetryProvider {
...
| `aws.smithy.kotlin.runtime.telemetry.metrics.aggregation.SdkTelemetryProvider` | `WARN` | Cardinality overflow, collection failures, buggy-exporter throws |
