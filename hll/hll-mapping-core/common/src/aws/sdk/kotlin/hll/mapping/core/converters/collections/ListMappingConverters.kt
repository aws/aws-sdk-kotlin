...
public fun <A, B> ListMappingMonoConverter(delegate: MonoConverter<A, B>): MonoConverter<List<A>, List<B>> = MonoConverter { it.map(delegate::convert) }
 ...
 */
@Suppress("ktlint:standard:function-naming")
public fun <L, R> ListMappingConverter(delegate: Converter<L, R>): Converter<List<L>, List<R>> = ListMappingMonoConverter(delegate.right) reversedBy ListMappingMonoConverter(delegate.left)
